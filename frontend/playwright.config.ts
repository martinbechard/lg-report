...
export default defineConfig({
  ...
  // Exercise the same launcher as users. An explicit price snapshot keeps
  // startup offline and capacity unknown for context-meter tests. The report
  // path isolates all selected samples.
  webServer: {
    command: '../.venv/bin/python -m agent_runtime --client angular --port 18765 --out ../reports/chat-e2e/simple_chat --prices ../tests/fixtures/accounting_prices.json --metadata-only',
