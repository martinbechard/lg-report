...
only the LLM. Ingestion remains separate: constructing the agent requires a
completed local index and never downloads or embeds the corpus.
...
def build_agent(model: BaseChatModel) -> CompiledStateGraph:
    """Open the fixed Wikipedia index and compile the agent using the supplied LLM.

    model is a configured provider adapter or offline model fixture. Opening the
    completed Chroma collection performs local storage reads; a missing or partial
    index raises an error directing the user to the ingestion command. No model
    invocation or corpus ingestion happens here. During execution, retrieved
    ...
    # or know Chroma's collection API. The shared loader checks index completion.
