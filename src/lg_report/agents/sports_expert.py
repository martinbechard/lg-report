"""Define the sports expert through its instructions and reference tool.
...
from lg_report.tools.domain_reference import search_sports_reference

NAME = "sports_expert"
# Registration metadata is intentionally separate from the prompt so the parent
# can advertise the domain without loading evidence into its own context.
DESCRIPTION = (
    "Specialist for questions about sports, with its own local reference lookup."
)
SYSTEM_PROMPT = (
    "You are the sports expert. Use search_sports_reference to retrieve evidence "
...
def build_agent(model: BaseChatModel) -> CompiledStateGraph:
    """Prepare a sports specialist to answer delegated questions with local evidence.
