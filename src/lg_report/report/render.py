...
from lg_report.platform.demo_meter import message_units, units
from lg_report.report.annotations import describe
from lg_report.report.collaboration import collaboration_diagrams
from lg_report.report.context import context_change
from lg_report.report.pricing import CATEGORIES, Prices, breakdown, summarize
...
def cost_chart(turns, prices):
    ...
    scale. Return None when FX is unavailable rather than labeling USD as EUR.
...
def render(run: Run, prices: Prices, destination: Path):
    ...
    template = env.from_string(
        files("lg_report.report").joinpath("templates/report.html").read_text()
    )
