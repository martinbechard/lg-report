...
`wikipedia.py` exposes the existing `tools/search_wikipedia.py` function through
FastMCP 4. The server opens a completed Chroma index once and publishes one
read-only tool, `search_wikipedia(query: str)`. The tool retains its 500-character
...
uv run python -m lg_report.mcp_servers.wikipedia
```
...
```json
{
  "mcpServers": {
    "wikipedia": {
      "command": "uv",
      "args": ["run", "--project", "/Users/martinbechard/dev/lg-report", "--locked", "python", "-m", "lg_report.mcp_servers.wikipedia"]
    }
  }
}
```
