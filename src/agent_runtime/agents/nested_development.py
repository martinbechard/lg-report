...
ROLE_SPECS = {
    "planner": (
        ...
        (
            ...
            "unchanged on subsequent calls: when stable_task is present, copy it "
            "exactly into task, preserving every string, punctuation mark and list "
            "order. Do not paraphrase, normalize spelling, or add requirements. "
            "Return exactly the allowed_action in "
...
def build_agent(role: str, parameters: dict):
    ...
    ).with_config(metadata={"report_agent": role})
    ...
    async def arun(request, config):
        return validate(
            await native.ainvoke(
                {"messages": prepare(request)},
                config=config,
            )
        )

    return RunnableLambda(run, afunc=arun).with_config(run_name=role)
