...
from agent_runtime.workflows.nested_contracts import (
    Assessment, Implementation, Limits, PlannerDecision, SupervisorDecision,
    bind_assessment, planner_action, require_action, supervisor_action,
)
...
def assessment(verdict="pass", candidate="candidate-1"):
    return {"candidate_id": candidate, "verdict": verdict,
            "findings": [] if verdict == "pass" else ["Fix the defect."],
            "evidence": "Scripted check.", "evidence_kind": "scripted"}


def approved():
    return {"status": "approved_for_testing",
            "artifact": {"candidate_id": "candidate-1", "source": "pass"}}


@pytest.mark.parametrize("rounds,verdict,expected", [
    (0, None, "code"), (1, "fail", "code"),
    (3, "fail", "escalate"), (3, "pass", "return"),
])
def test_inner_guard(rounds, verdict, expected):
    assert supervisor_action(None if verdict is None else assessment(verdict), rounds, 3) == expected


@pytest.mark.parametrize("cycles,verdict,expected", [
    (1, "fail", "code"), (3, "fail", "escalate"), (3, "pass", "finish"),
])
...
def test_coder_notes_are_not_part_of_handoff():
    response = Implementation(candidate_id="candidate-1", source="pass", working_notes="private")
    assert response.handoff().model_dump() == {"candidate_id": "candidate-1", "source": "pass"}
...
def replay_policy(scenario, max_review_rounds=3, max_coding_cycles=3):
    ...
    """
    queues = {key: deque(values) for key, values in script({
        "scenario": scenario, "max_review_rounds": max_review_rounds,
    ...
    for _ in range(3 * max_coding_cycles + 3):
        ...
        if action in {"finish", "escalate"}:
            assert not any(queues.values()), "Unconsumed decisions would misdescribe the run"
        ...
        for rounds in range(max_review_rounds + 1):
            ...
            require_action(take("coding_supervisor", SupervisorDecision)["action"], command)
            if command != "code":
                coding = {"status": "approved_for_testing" if command == "return" else "blocked",
                          "artifact": artifact}
                testing = None
                break
            attempts += 1
            implementation = Implementation.model_validate(take("coder", Implementation))
...
def test_default_story_contains_both_repair_paths():
    ...
    assert counts == {"planner": 5, "coding_supervisor": 5, "coder": 3, "reviewer": 3, "tester": 2}
...
def test_authored_fixture_defects_are_real(source, expected):
    ...
    namespace = {}
    exec(compile(source, "<trusted-demo-fixture>", "exec"), namespace)
...
def test_fixed_fixture_preserves_order_and_does_not_mutate():
    namespace = {}
    exec(compile(SOURCE_V3, "<trusted-demo-fixture>", "exec"), namespace)
