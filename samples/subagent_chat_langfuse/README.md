...
owns its application entry point and configuration; `lg_report.workflows.subagent_chat` composes the shared agents, and
`subagent_chat/test_case.py` owns prompts and both model fixtures.
...
```sh
uv sync --locked
uv run python -m samples.subagent_chat_langfuse.app --public-trace
```
...
`app.py` passes the shared graph factory to `lg_report.platform.langfuse_runtime.launch`.
That runtime handles configuration, project authentication, trace visibility,
...
The shared runtime now consumes a StaticClient through the common Conversation
