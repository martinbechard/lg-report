...
from agent_runtime.workflows.nested_contracts import Limits
...
def script(options=None):
    ...
    limits = Limits(options.get("max_review_rounds", 3), options.get("max_coding_cycles", 3))
    queues = {name: [] for name in ("planner", "coding_supervisor", "coder", "reviewer", "tester")}
    current = 0

    def plan(action, note):
        queues["planner"].append({"action": action, "task": deepcopy(TASK), "note": note})

    def supervise(action, directive):
        queues["coding_supervisor"].append({
            "action": action, "directive": "CODING_ONLY_DETAIL: " + directive,
        })
    ...
    def develop(source):
        ...
        current += 1
        queues["coder"].append({
            "candidate_id": f"candidate-{current}", "source": source,
            "working_notes": "CODING_ONLY_DETAIL: private implementation discussion.",
        })

    def assess(role, verdict, findings, evidence):
        queues[role].append({
            "candidate_id": f"candidate-{current}", "verdict": verdict,
            "findings": findings, "evidence": evidence, "evidence_kind": "scripted",
        })

    def accepted_review():
        assess("reviewer", "pass", [], "Scripted review approval; not proof of correctness.")
        supervise("return", "Return the review-approved artifact to the planner.")
        plan("test", "Send the candidate to the tester; review alone is not acceptance.")

    plan("code", "Dispatch the stable task to the coding supervisor.")
    if scenario == "rework":
        if limits.max_review_rounds < 2 or limits.max_coding_cycles < 2:
            raise ValueError("The rework story requires at least two rounds and two cycles")
        supervise("code", "Implement the task.")
        develop(SOURCE_V1)
        assess("reviewer", "fail", ["Remove duplicate normalized tags."],
        ...
        develop(SOURCE_V3)
        accepted_review()
        assess("tester", "pass", [], "Scripted acceptance approval; no test runner was called.")
        ...
        plan("escalate", "The coding supervisor returned a blocked result; do not test.")
        ...
        for cycle in range(limits.max_coding_cycles):
            ...
            develop(SOURCE_V2)
            accepted_review()
            assess("tester", "fail", ["Whitespace-only input still produces an empty tag."],
            ...
            if cycle + 1 < limits.max_coding_cycles:
                ...
                plan("escalate", "Stop: the maximum test-fix coding cycles have been used.")
